...
export default function GroupSettings({
  ...
  onDeleted: () => void;
  onInspectOfflineData: () => Promise<
    MutationResult<OfflineGroupDataSummary>
  >;
  onDeleteOfflineData: () => Promise<{ ok: boolean; message?: string }>;
  onOfflineDataDeleted: () => void;
}) {
  const {
    syncStatus,
    changeMyMember,
  ...
  const remoteControlsUnavailable = isOffline || remoteDisabled;
  const controlsDisabled =
    disabled || remoteControlsUnavailable || busy !== null;
  ...
  const handleDeleteOfflineData = async () => {
    onActionStart();
    setBusy("deleteOfflineData");
    try {
      const inspected = await onInspectOfflineData();
      if (!inspected.ok) {
        onNotice(inspected.message, "error");
        return;
      }
      const { cachedSnapshotCount, draftCount, pendingCount: storedPendingCount } =
        inspected.data;
      if (
        !window.confirm(
          `この端末のキャッシュ${cachedSnapshotCount}件、入力下書き${draftCount}件、未同期の支払い${storedPendingCount}件を削除します。Supabase上の正式データは削除されません。`
        )
      ) {
        return;
      }

      const result = await onDeleteOfflineData();
      if (!result.ok) {
        onNotice(
          result.message ?? "この端末の保存データを削除できませんでした。",
          "error"
        );
        return;
      }
      onNotice("この端末の保存データを削除しました。", "success");
      onOfflineDataDeleted();
    } catch {
      onNotice("この端末の保存データを削除できませんでした。", "error");
    } finally {
      setBusy(null);
    }
  };
      ...
      <div className="space-y-5 border-t border-blue-100 px-4 py-5">
        <div>
          ...
          <select
            ...
            aria-describedby={
              remoteControlsUnavailable ? "group-offline-reason" : undefined
            }
            title={remoteControlsUnavailable ? disabledReason : undefined}
        ...
        <section
          aria-labelledby="offline-data-settings-heading"
          className="border-t border-blue-200 pt-4"
        >
          <h3
            id="offline-data-settings-heading"
          ...
          <ActionButton
            ...
            loading={busy === "deleteOfflineData"}
            loadingLabel="削除中..."
            onClick={() => void handleDeleteOfflineData()}
          >
            端末データを削除
          </ActionButton>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-2">
                <ActionButton
                  ...
                  loadingLabel="発行中..."
                  aria-describedby={
                    remoteControlsUnavailable
                      ? "group-offline-reason"
                      : undefined
                  }
                  title={
                    remoteControlsUnavailable ? disabledReason : undefined
                  }
                ...
                <ActionButton
                  ...
                  loadingLabel="変更中..."
                  aria-label="招待リンク"
                  aria-pressed={snapshot.group.inviteEnabled}
                  aria-describedby={
                    remoteControlsUnavailable
                      ? "group-offline-reason"
                      : undefined
