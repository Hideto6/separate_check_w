...
- グループ、メンバー、支払い記録は React Context で管理し、ブラウザの `localStorage` に保存する。
- 金額は日本円の整数として扱い、サーバーやデータベースは使用しない。
...
- `localStorage` は Client Component のブラウザ実行時だけ参照する。初回レンダー中や Server Component から参照しない。
- 保存値を復元し終えるまで `isHydrated` で画面遷移と永続化を抑止し、未復元の初期値で保存データを上書きしない。
- 復元値は利用前に実行時検証する。不正な値はアプリ状態へ取り込まない。
- `resetGroup` はメモリ上のグループ名、メンバー、記録と、対応するすべての保存キーを消去する。
- 保存データの形やキーを変える場合は、型、検証、復元、保存、リセットを同じ変更で更新し、既存データの互換方法または明示的な破棄方法を定める。
