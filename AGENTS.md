...
- `localStorage` は匿名認証セッション、作成者の招待トークン、この端末で最後に開いた共有グループ1件のID・名前だけに使い、Client Componentのブラウザ実行時だけ参照する。
- Supabaseから取得したスナップショットと直近グループ保存値は利用前に実行時検証し、不正な値をアプリ状態へ取り込まない。
