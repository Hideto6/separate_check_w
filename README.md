...
`localStorage`には匿名認証セッション、作成者用の招待トークン、直近グループ1件のIDと名前だけを保存します。グループ本体の正しいデータはSupabaseにあり、端末へスナップショットやダミーデータは保存しません。共同編集対応前の旧端末データはホーム表示時に削除されます。
...
通信切断中は読み込み済みのデータをそのまま表示し、更新操作だけを無効にします。再接続時は自動で最新状態を再取得し、オフライン中の書き込みキューは作りません。また、支払いと精算取り消しには確認を表示し、失敗理由を日本語で案内します。
